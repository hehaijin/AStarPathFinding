...
/**
 *This is the class for building up the graph, look for the path between two strings and print the path.
 *It's main method is the entry of the program.
 *@author Haijin He 
*/
 ...
 * First read the dictionary into a file.
 * For each word in the dictionary, generate all its neighbors,
 * scan the dictionary if any word matches the neighbors, and add edge.
 * @param path an string for the path of the dictionary file. 
 */
	 ...
	 * @param a  one of the two nodes for adding edge.
	 * @param b  one of the two node for adding edge
	 */
	...
	/**
	 * Method used for displaying a small graph.
	 * used for testing. 
	 */
...
/**
 * Method for showing all adjacent edge for a node containing string s.
 * used for testing.
 ...
 */
	...
	/**
	 * Finds the path between source node and destination node.
	 * First check if string s and d really have a node.
	 * If not, print an error message and return.
	 * The precedure uses A* algorithm.
	 * A priority queue is used, based on the addition of cost_so_far and cost_estimate.
	 * cost_so_far is current distance from source
	 * cost_estimate is the estimate of remaining distance from destination. Here it is simply the length difference.
	 * Each step, takes a node from the priority queue, and recalculate all of it's neighbors.
	 * @param s  the source node with string s
	 * @param d the destination node with string d
	 */
	 ...
	 * @param s source node with string s
	 * @param d destination node with string d
	 */
	...
	/**
	 * Prints the path from source node to destination node.
	 * Starts from destination node, push every previous node into a stack,
	 * till it reach source node.
	 * Then pops all nodes out from the stack.
	 * @param s source node with string s
	 * @param d destination node with string d
	 */
 ...
 */
 ...
 public static void main(String[] args)
 {
	...
	for(int i=1;i<args.length-1;i=i+2)
	{
		...
		if(args.length%2==0) System.out.println("Input words must be in pair!");
