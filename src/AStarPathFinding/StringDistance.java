...
/**
 * This class contains several static methods for calculating distance for two strings
 * @author Haijin He
 *
 */
...
public class StringDistance 
{
   ...
   * @param s1 String s1
   * @param s2 String s2
   * @return The hamming distance between s1 and s2. return -1 if their length are not equal.
   */
	...
	{ 
		...
		if(s1.length()==s2.length() && hamming(s1,s2)==1) return true;
		if(s1.length()< s2.length()) 
		{
			 for(int i=0;i<s2.length();i++)
			 { 
				 if(s1.equals(s2.substring(0,i)+s2.substring(i+1))) return true;
			 }
		...
		else 
		{
			for(int i=0;i<s1.length();i++)
			 {
				 if(s2.equals(s1.substring(0,i)+s1.substring(i+1))) return true;
			 }
		}
	 ...
	 * @param s1 String s1
	 * @param s2 String s2
	 * @return  The estimated distance between s1 and s2. Here it's simply the length differnce.
	 */
